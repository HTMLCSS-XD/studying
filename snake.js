...
var map={width:1456,height:728};
var box={width:50, height:50};
var nums={
	hNum:map.width/(box.width+2),
	vNum:map.height/(box.height+2)
};
var snake=[];//保存蛇身体
var other=[];//除蛇外其他部分
var wall=[];//墙
...
function snakeMove(){
	...
	for(var i=1;i<snake.length;i++){
		if(headId==snake[i].id){
			alert("Game Over!长度为："+snake.length);
	...
	if(head.className=="food"){
		//吃到食物，原地不动，蛇头+1；并刷新食物
        if(snake.length%5==1&&wall.length<=10){
       var windex=other[Math.floor(Math.random()*other.length)];
        wall.push(windex);
        windex.className="wall";
    }
		showFood();		
	}else{
		snake[0].className="";
